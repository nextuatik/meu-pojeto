...
# --- NOVOS AGENDAMENTOS ---
@app.route('/admin/novos_agendamentos')
def novos_agendamentos():
    conn = conectar_db()
    resultados = conn.execute('SELECT COUNT(*) as total FROM horarios WHERE lido=0').fetchone()
    conn.close()
    return jsonify({'total': resultados['total']})

@app.route('/admin/marcar_lidos')
def marcar_lidos():
    conn = conectar_db()
    conn.execute('UPDATE horarios SET lido=1 WHERE lido=0')
    conn.commit()
    conn.close()
...
def enviar():
    ...
    conn.execute(
        ...
        (nome_cliente, telefone, servico, barbeiro_nome, data, hora, observacoes, lido)
        VALUES (?, ?, ?, ?, ?, ?, ?, 0)''',
    ...
    socketio.emit('novo_agendamento', {
        'cliente': nome,
        'hora': hora,
        'data': data_agendada,
        'barbeiro': barbeiro
